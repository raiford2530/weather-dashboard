$(document).ready(function () {
  ...
  const lastCitySearched = localStorage.getItem("lastCitySearched");
  if (lastCitySearched) {
  ...
  function getWeather(city, addHistory) {
    ...
    $.ajax({
      ...
      const weatherIcon = getWeatherIcon(response.weather[0].icon);
      ...
      $("#city").append(
        `<img class="weather-icon" alt="${response.weather[0].description}" src="${weatherIcon}>`
      );
  ...
  function getFiveDayForecast(lat, lon) {
    ...
    $.ajax({
      ...
      for (var i = 1; i < 6; i++) {
        ...
        var forecastHTML = `<li>
                              <div>
                                <p class='font-weight-bold'>${date.toLocaleDateString()}</p>
                                <img src="${weatherIcon}" alt="${forecast.weather[0].description}" class="five-day-weather-icon">
  ...
  function getUVIndex(lat, lon) {
    const url = root + `uvi?lat=${lat}&lon=${lon }&appid=${token}`;
